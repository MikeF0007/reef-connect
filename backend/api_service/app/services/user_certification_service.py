...
class UserCertificationService:
    ...
    async def get_user_certifications(self, user_id: UUID) -> List[UserCertificationResponse]:
        """Get all certifications for a user."""
        certifications = await self.repository.get_user_certifications(user_id)
        return [UserCertificationResponse.model_validate(cert) for cert in certifications]

    async def add_user_certification(
        self, user_id: UUID, data: UserCertificationCreate
    ) -> UserCertificationResponse:
        """Add a new certification for a user."""
        cert_id = await self.repository.add_user_certification(user_id=user_id, **data.model_dump())
    ...
    async def delete_user_certification(self, certification_id: UUID, user_id: UUID) -> None:
